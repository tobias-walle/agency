# cli-attach-follow Specification

## Purpose
TBD - created by archiving change attach-follow-tui-focus. Update Purpose after archive.
## Requirements
...
When the focused task has no running session, the CLI MUST present a minimal overlay using raw mode and the alternate screen that prompts to start, and MUST switch to an attach child once the session exists.
...
### Requirement: Real-time overlay updates from daemon
