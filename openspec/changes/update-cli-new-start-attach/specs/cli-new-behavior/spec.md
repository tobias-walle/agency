# cli-new-behavior Change Spec

## Purpose
Define the CLI behavior for `agency new` so that the default flow creates a task, starts its session, and attaches without opening an editor, while preserving explicit draft and no-attach flows.
...
## Notes
- TUI behavior for creating and starting tasks is specified separately in `specs/tui-start-behavior/spec.md` and MUST remain consistent with this CLI behavior where applicable.
- Worktree and bootstrap creation remain lazy at attach or bootstrap time and are out of scope for this change.
