...
use crate::agent::{build_env, resolve_action, substitute_tokens, AgentAction};
  ...
  module
    .register_method("task.start", |params, _ctx: &PathBuf, _ext| -> RpcResult<serde_json::Value> {
      ...
      let env_pairs: Vec<(&str, &str)> = env_map
        ...
        .collect();
      info!(
        event = "task_start_spawn_prepare",
        id,
        slug = %slug,
        agent = ?task.front_matter.agent,
        program = %program,
        args_len = substituted_args.len(),
        env_vars = env_pairs.len(),
