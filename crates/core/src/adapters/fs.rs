...
/// Return path to the `.agency` folder inside the given project root
pub fn agency_dir(project_root: &Path) -> PathBuf {
  project_root.join(".agency")
}

/// Standard subpaths under `.agency`
pub fn logs_path(project_root: &Path) -> PathBuf {
  agency_dir(project_root).join("logs.jsonl")
}

pub fn tasks_dir(project_root: &Path) -> PathBuf {
  agency_dir(project_root).join("tasks")
}

pub fn worktrees_dir(project_root: &Path) -> PathBuf {
  agency_dir(project_root).join("worktrees")
}
...
/// Ensure the `.agency` layout exists (directories are created if missing)
pub fn ensure_layout(project_root: &Path) -> std::io::Result<()> {
  ...
  fs::create_dir_all(agency_dir(project_root))?;
  Ok(())
}
...
mod tests {
  ...
  fn layout_paths() {
    ...
    let root = td.path();
    assert_eq!(agency_dir(root), root.join(".agency"));
    assert_eq!(logs_path(root), root.join(".agency/logs.jsonl"));
    assert_eq!(tasks_dir(root), root.join(".agency/tasks"));
    assert_eq!(worktrees_dir(root), root.join(".agency/worktrees"));
  }
  ...
  fn worktree_path_is_under_worktrees() {
    ...
    assert_eq!(p, root.join(".agency/worktrees/42-abc"));
  }
}
