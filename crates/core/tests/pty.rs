...
async fn pty_reattach_replays_scrollback_tail() {
  ...
  let att: RpcResp<PtyAttachResult> = rpc_call(&env.sock, "pty.attach", Some(attach_params.clone())).await;
  assert!(att.error.is_none());
  let attachment_id = att.result.unwrap().attachment_id;

  // Send input to generate output
  let _: RpcResp<Value> = rpc_call(&env.sock, "pty.input", Some(json!({
    "attachment_id": attachment_id,
    "data": "echo 'line1'\necho 'line2'\necho 'line3'\n"
  }))).await;
  ...
  assert!(collected.contains("line1") && collected.contains("line2") && collected.contains("line3"));

  // Detach
  let _: RpcResp<Value> = rpc_call(&env.sock, "pty.detach", Some(json!({
    "attachment_id": attachment_id
  }))).await;
  ...
  let r2: RpcResp<PtyReadResult> = rpc_call(&env.sock, "pty.read", Some(json!({
...
async fn pty_read_wait_ms_returns_on_data() {
  ...
  let task = tokio::spawn(async move {
    ...
    let r: RpcResp<PtyReadResult> = rpc_call(&sock_clone, "pty.read", Some(json!({
...
async fn pty_tick_applies_input_and_returns_output() {
  ...
  let r: RpcResp<PtyReadResult> = rpc_call(&env.sock, "pty.tick", Some(json!({
    "attachment_id": attachment_id,
    "input": "echo hello\n",
    "resize": null,
    "max_bytes": 8192usize,
  ...
  let data = r.result.unwrap().data;
  assert!(data.to_lowercase().contains("hello"), "tick should return echoed output, got: {}", data);
