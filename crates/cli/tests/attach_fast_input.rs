...
fn attach_handles_fast_small_chunks_and_final_detach() {
  ...
  Command::cargo_bin("agency")
    ...
    .env("AGENCY_SOCKET", sock.as_os_str())
    .current_dir(&root)
    .args(["new", "--agent", "fake", "--draft", "feat-fast"])
    .assert()
    .success();
...
#[test]
fn attach_fast_burst_full_echo() {
  let td = tempfile::tempdir().unwrap();
  let root = td.path().to_path_buf();
  let sock = td.path().join("agency.sock");
  ...
  Command::cargo_bin("agency")
    ...
    .env("AGENCY_SOCKET", sock.as_os_str())
    .current_dir(&root)
    .args(["new", "--agent", "fake", "--draft", "feat-burst"])
    .assert()
    .success();
  ...
  attach
    ...
    .env("AGENCY_DETACH_KEYS", "ctrl-p,ctrl-q")
    .current_dir(&root)
    .args(["attach", "feat-burst"])
    .stdin(Stdio::piped())
    .stdout(Stdio::piped())
    .stderr(Stdio::piped());
  let mut child = attach.spawn().expect("spawn attach");

  // Write a rapid burst of characters in tiny chunks, then detach
  {
    let stdin = child.stdin.as_mut().expect("stdin");
    let burst = "abcdefghijklmnopqrstuvwxyz0123456789";
    let cmdline = format!("echo {}\n", burst);
  ...
  assert!(stdout.contains("Attached. Detach:"), "stdout: {}", stdout);
  assert!(
    stdout.contains("abcdefghijklmnopqrstuvwxyz0123456789"),
    "stdout: {}",
    stdout
  );
  assert!(stderr.contains("detached"), "stderr: {}", stderr);
