...
fn daemon_status_when_running_prints_running() {
  // Spin up a temporary daemon bound to a temp socket and set AGENCY_SOCKET for the CLI.
  // We rely on the core daemon test helper shape similar to core tests.
  // To avoid cross-crate async startup here, we just assert that when AGENCY_SOCKET is
