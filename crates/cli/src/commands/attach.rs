use crate::{args, rpc::client, stdin_handler, term_reset, util::{daemon_proc::ensure_daemon_running, detach_keys::parse_detach_keys, task_ref::parse_task_ref}};
...
pub fn attach_interactive(args: args::AttachArgs) {
  ...
  let detach_cfg = std::env::var("AGENCY_DETACH_KEYS").ok().or_else(|| cfg.pty.detach_keys.clone());
  let detach_seq = detach_cfg.as_deref().map(parse_detach_keys).unwrap_or_else(|| vec![("Q".chars().next().unwrap() as u8) & 0x1f]);

  let (cols, rows) = size().unwrap_or((80, 24));
  let rt = tokio::runtime::Builder::new_multi_thread().enable_io().enable_time().worker_threads(2).build().unwrap();

  let attach_res = rt.block_on(async { client::pty_attach_with_replay(&sock, &root, tref, rows, cols, !args.no_replay).await });
  let attachment_id = match attach_res {
    Ok(r) => r.attachment_id,
    Err(e) => { eprintln!("{}", crate::util::errors::render_rpc_failure("attach", &sock, &e)); std::process::exit(1); }
  };
  ...
  std::thread::spawn(move || {
    ...
    loop {
      if let Ok((cols, rows)) = size() && (rows != last_rows || cols != last_cols) {
        last_rows = rows;
        last_cols = cols;
    ...
    loop {
      ...
      while let Ok(msg) = rx.try_recv() {
        match msg {
          ...
          stdin_handler::Msg::Binding(id) if id == "detach" => { want_detach = true; drained_bindings += 1; }
          _ => {}
        }
      }
      ...
      let rr = client::session::pty_read_wait(&session, &sock, &attachment_id, Some(8192), Some(wait_ms)).await;
      match rr {
        Ok(r) => {
          debug!(event = "cli_pty_read_result", bytes = r.data.len(), eof = r.eof, wait_ms, "read from PTY");
          if !r.data.is_empty() { let _ = stdout.write_all(r.data.as_bytes()); let _ = stdout.flush(); }
          if r.eof { break; }
        }
        Err(e) => { debug!(event = "cli_pty_read_error", error = %e, "read error"); break; }
      }

      if want_detach { debug!(event = "cli_detach_requested", "detach binding triggered"); detached = true; break; }

      if start.elapsed() < Duration::from_millis(1) { tokio::time::sleep(Duration::from_millis(1)).await; }
    }
  });
