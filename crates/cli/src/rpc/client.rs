...
impl PtySession {
  ...
  pub async fn rpc_call(&self, sock: &Path, method: &str, params: Option<serde_json::Value>) -> Result<serde_json::Value> {
...
pub mod session {
  ...
  pub async fn pty_input(session: &PtySession, sock: &Path, attachment_id: &str, data: &[u8]) -> Result<()> {
    let params = serde_json::to_value(PtyInputParams {
      attachment_id: attachment_id.to_string(),
      data: String::from_utf8_lossy(data).to_string(),
    })?;
  ...
  pub async fn pty_resize(session: &PtySession, sock: &Path, attachment_id: &str, rows: u16, cols: u16) -> Result<()> {
    let params = serde_json::to_value(PtyResizeParams {
      attachment_id: attachment_id.to_string(),
      rows,
      cols,
