...
The Agency v1 PRD defines a single-user daemon that orchestrates AI CLI agents in isolated Git worktrees with a JSON-RPC API, MCP interface, and a thin CLI.
We want a lean, readable architecture to ship an MVP quickly without over-fragmenting the codebase.

Related PRD: [PRD-1-agency-v1.md](../prd/PRD-1-agency-v1.md)
...
2. Packaging: Ship a single binary for all commands (`agency`), including the CLI and an `mcp` subcommand that starts the MCP server.
...
6. PTY detach UX: Default detach sequence Ctrl-q; configurable via config `pty.detach_keys` and env `AGENCY_DETACH_KEYS` (no CLI flag). Do not override Ctrl-C by default; print a detach hint on attach.
...
The `agency` binary provides all user commands and also offers an `mcp` subcommand to start the MCP server that bridges to the daemon API.
All orchestration and side effects live in `core`; clients remain thin.

## Workspace Structure (high-level)

```
agency/                     # workspace root
  crates/
  ...
  apps/
    agency/                 # single binary entrypoint (CLI + `mcp` subcommand)
```
...
crates/core/
  src/
    ...
    adapters/
      ...
      fs.rs                # paths, .agency layout utilities
    logging/
...
crates/cli/
  src/
    lib.rs               # public CLI API used by apps/agency
...
- Logging: use `tracing`; attach `task_id`, `session_id` to spans; JSON goes to `./.agency/logs.jsonl` via `logging::init()`.
...
- `core`
  ...
  - Config loading/merging (global + project) and tracing setup (JSON logs to `.agency/logs.jsonl`).
- `cli`
...
- `apps/agency`
  ...
  - Dispatch CLI commands; provide `agency mcp` to run the MCP server.

## Runtime Flows (concise)

- Daemon startup: ensure socket path from `AGENCY_SOCKET` (platform defaults per PRD), acquire file lock, init tracing and config, start JSON-RPC server.
...
- Global config: `~/.config/agency/config.toml`; Project config: `./.agency/config.toml`; merged with project overriding global.
...
- Socket path: `AGENCY_SOCKET` with platform-specific defaults from PRD; prefer `dirs::runtime_dir()` over `dirs::data_dir()` for the default socket location; Windows not supported.

## Logging & Observability

- `tracing` with JSON formatter writes to `./.agency/logs.jsonl`.
...
- The workspace will start with a maximum of four crates: `core`, `cli`, `mcp`, and `apps/agency` (binary). This keeps boundaries clear while minimizing boilerplate.
- The CLI is a thin JSON-RPC client; all orchestration remains in `core`.
- Logging remains centralized with `tracing` to `./.agency/logs.jsonl`.
...
- Multiple binaries (separate `agency-daemon`, `agency-cli`, `agency-mcp`). Rejected to minimize distribution complexity and cognitive load.
