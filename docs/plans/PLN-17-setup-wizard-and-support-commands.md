...
- Add `agency init-project` to scaffold `.agency/` with confirmation and summary.
...
- Commands are defined and dispatched in `crates/agency/src/lib.rs:1`. No `setup`, `defaults`, or `init-project` exists yet.
...
- Add `setup` subcommand that:
  ...
  - Prints a concise, colored summary and hints (`agency defaults`, `agency init-project`, `agency --help`).
...
- Copy maintenance and colors:
  ...
  - Keep setup-specific strings in `copy/setup.rs` with token placeholders for easy updates.
...
- New files
  ...
  - `crates/agency/src/commands/init_project.rs` — scaffold `.agency/` files
  - `crates/agency/src/utils/wizard.rs` — generic wizard helpers (logo, prompts, theme)
  - `crates/agency/src/copy/setup.rs` — setup-specific copy (strings, explanations)
...
- [ ] Tests: CLI setup flow creates global config
  - Add integration tests in `crates/agency/tests/cli.rs`:
    ...
    - Assert output mentions `agency defaults` and `agency init-project`.
- [ ] Tests: Re-running setup updates existing config and warns
...
- [ ] Tests: `agency defaults` prints embedded TOML
  - Assert header line and that body contains keys from `defaults/agency.toml` (e.g., `[agents.claude]`).
- [ ] Tests: `agency init-project` scaffolds files after confirmation
...
- [ ] Implement `commands/setup.rs`
  ...
  - Print final summary + tips (`agency defaults`, `agency init-project`, `agency --help`, quick starts).
- [ ] Wire CLI and first-run path
...
4. Where should `init-project` place the initial `.agency/agency.toml` content? Default: create an empty file to encourage local overrides; guide users to `agency defaults` for reference.
